...
contract StrategyHybrid is Ownable, ReentrancyGuard, Strategy {
    ...
    /// @dev Throws if called by any account other than the orbit.
    modifier onlyOrbit() {
        require(isOrbit(), "caller is not the orbit");
        _;
    }

    /// @dev Returns true if the caller is the current orbit.
    function isOrbit() public view returns (bool) {
        return msg.sender == orbit;
    }
