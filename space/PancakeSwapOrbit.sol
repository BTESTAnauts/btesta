...
contract PancakeSwapOrbit is Ownable, ReentrancyGuard, Orbit {
    /// @notice Libraries
    ...
    constructor(
        ...
        syrupBar = address(masterChef.syrup());
        cakeToken = address(masterChef.cake());
        addStrat = _addStrat;
        liqStrat = _liqStrat;
        ...
        syrupBar.safeApprove(address(router), uint256(-1)); // 100% trust in the router
        cakeToken.safeApprove(address(router), uint256(-1)); // 100% trust in the router
    ...
    function refuel() public onlyEOA nonReentrant {
        ...
        uint256 reward = cakeToken.balanceOf(address(this));
        if (reward == 0) return;
        // 2. Send the reward bounty to the caller.
        uint256 bounty = reward.mul(reinvestBountyBps) / 10000;
        cakeToken.safeTransfer(msg.sender, bounty);
